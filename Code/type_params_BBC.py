...
def func_preds(model,xtest):
    preds = model.predict(np.array(xtest), verbose=False)
...
for i in range(len(name_str)):
    ...
    if name_str[i]=='MultiClass':
        for j in range(len(type_params)):
            start = time.time()
            df_metrics=main_multi(name_models[i](dic_params[name_str[i]][j][0],dic_params[name_str[i]][j][1]),np.array(xtest),50)
            end = time.time()
        ...
        for j in range(len(type_params)):
            start = time.time()
            df_metrics=main_softmax(name_models[i](dic_params[name_str[i]][j][0],dic_params[name_str[i]][j][1]),np.array(xtest),50)
            end = time.time()
            df_metrics['model']=name_str[i]
            df_metrics['time']=end - start
