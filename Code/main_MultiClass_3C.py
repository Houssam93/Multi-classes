...
if name_data=="Tweets_3C" :
    exec(open('prepare_data_Tweets_3C.py').read())

    input_dime=512

exec(open('Models_MultiClass_3C.py').read())
...
def train_model_softmax(name_model,epochs):
    ...
    model.fit(xtrain, [ytrain.transpose()[0],ytrain.transpose()[1],ytrain.transpose()[2]],
                epochs=epochs, validation_data=(xvalid, [yvalid.transpose()[0],yvalid.transpose()[1],yvalid.transpose()[2]]),  verbose=False, callbacks=[EarlyStopping(patience=5)])
...
def main_multi(name_model,xtest,epochs):
    ...
    if name_data =='BBC' or name_data =='BBCSport' or name_data =='Tweets_5C' or name_data =='Tweets_3C' :
...
'ModelOneVsRest_All_shared_2Per2','ModelOneVsRest_SharedPrivate_2Per2']
print(name_str)
