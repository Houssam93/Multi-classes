...
#name_data="Cogalex"
name_data="Complaints"
print(name_data)
if name_data =='Complaints' :
    exec(open('prepare_data_complaints.py').read())
    input_dime=512

exec(open('Models_MultiClass_16C.py').read())
...
def train_model_softmax(name_model,epochs):
    ...
    model.fit(xtrain, [ytrain.transpose()[0],ytrain.transpose()[1],ytrain.transpose()[2]
                       ,ytrain.transpose()[3],ytrain.transpose()[4],ytrain.transpose()[5],ytrain.transpose()[6]
                       ,ytrain.transpose()[7],ytrain.transpose()[8],ytrain.transpose()[9],ytrain.transpose()[10]
                       ,ytrain.transpose()[11],ytrain.transpose()[12],ytrain.transpose()[13],ytrain.transpose()[14],ytrain.transpose()[15]],
                epochs=epochs, validation_data=(xvalid, [yvalid.transpose()[0],yvalid.transpose()[1],yvalid.transpose()[2]
                       ,yvalid.transpose()[3],yvalid.transpose()[4],yvalid.transpose()[5],yvalid.transpose()[6]
                       ,yvalid.transpose()[7],yvalid.transpose()[8],yvalid.transpose()[9],yvalid.transpose()[10]
                       ,yvalid.transpose()[11],yvalid.transpose()[12],yvalid.transpose()[13],yvalid.transpose()[14]
                       ,yvalid.transpose()[15]]),  verbose=False, callbacks=[EarlyStopping(patience=5)])
